...
public class ClientMachine {
    ...
    public static void main(String[] args) {
        try {
            IDao<Salle> daoSalle = (IDao<Salle>) Naming.lookup("rmi://localhost:1099/dao-salle");
            daoSalle.findAll();
                                 /*

            Salle salle=new Salle("DDD");
            daoSalle.create(salle);
            IDao<Machine> daoMachine = (IDao<Machine>) Naming.lookup("rmi://localhost:1099/dao-machine");
            daoMachine.create(new Machine("RT55", "TOSHIBA", 7000,salle));
           */         
